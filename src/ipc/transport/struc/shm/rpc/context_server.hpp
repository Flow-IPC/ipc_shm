...
public:
  ...
  static_assert(Session_server_obj::Server_session_obj::Channel_obj::S_HAS_NATIVE_HANDLE_PIPE,
                "Context_server, per its protocol with the opposing Client_context, receives a native handle -- "
                  "the stream-socket underlying each capnp-RPC conversation -- over the first init-channel; "
                  "therefore the supplied Session_server_t must be configured to emit Native_handle-bearing "
   ...
   *   - those emitted at the very start of the internal protocol, once said session is established
   *     (as of this writing, in practice, by transport::Native_socket_stream::async_receive_native_handle();
   *     see its doc header);
                                           ...
                                           Transport_method_func&& transport_method_func)
{
  ...
  struct Session_accept_result
  {
    ...
    /* .front() is reserved for the capnp-RPC transport-handle exchange (see below).  We will get it out of there
     * before giving them Accept_result with the rest of these (if any). */
    Channels m_init_channels_by_cli_req;
  };

  /* Bundles the objects involved in receiving the capnp-RPC transport handle over the client's first
   * init-channel (see continuation below); kept alive via shared_ptr through the async op + continuation. */
  struct Transport_hndl_receive
  {
   ...
   *   -# Thread U: Got signalled, so continue: We have the Server_session and any init-channels filled out.
   *      Per protocol the client now sends the capnp-RPC transport handle over init-channel 0; so kick off
   *      an async-receive of that handle.  Its completion handler fires in a channel-internal thread; so
   *      bridge back to thread U via another cross-thread fulfiller (same pattern as step 2).
   *      - Could we simply assume that, upon successful asynchronous session-accept, the native-handle would
   *        arrive quickly (hence we could use a regular promise+future -- making the code shorter and basically
   *        synchronous in this step)?  Yes, if we trust the opposing side to be well-behaved... but no, in that
   *        while (as explained elsewhere) we do trust their being non-hostile and even non-buggy (Flow-IPC-wise),
   *        they could still get stuck for some reason, maybe not Flow-IPC-related.  So: it's better not to assume
   *        the async_receive_native_handle() will be instant.
   *   -# Thread U: Got the transport handle; destroy the by-now-purposeless channel; bundle stuff into a new
   *      Server_context (which will construct the Vat_network from the Server_session and that handle; and own
   *      those from now on); then slightly bundle *that* guy and remaining init-channels into a new
   *      Accept_result (just a simple struct).
   *   -# Thread U: Fulfill the promise from step 1 by loading it with the Accept_result.
   *
   * The error path is similar enough; just steps 3-5 detect the problem (bad Error_code from session-accept or
   * handle-receive; or OK Error_code, but Server_context fails to be created, presumably Vat_network setup being
                              ...
                              mutable
  {
    ...
    /* Per our protocol with the opposing Client_context (see its class doc header @todo regarding
     * Protocol_negotiator), chan_for_rpc exists for one purpose: the client sends -- over it, to us -- a single
     * native handle: one end of a pre-connected stream-socket pair, over which the two Session_vat_network peer
     * objects (theirs and ours) shall subsequently transmit their (small) messages.  (See
     * Client_context::sync_connect_impl() for the counterpart logic and further discussion; e.g., why not simply
     * use chan_for_rpc's own low-level transport.)  So: async-receive that handle; destroy the channel (its work
     * is then done); and use the handle to cook-up the Server_context (which feeds it to its Session_vat_network).
     *
     * Mechanically: we receive via the async-I/O API (less fuss than the sync_io-pattern API for a one-off op);
     * its completion handler fires in a channel-internal thread; so -- exactly as with the session-accept above --
     * we bridge back to thread U via a cross-thread fulfiller and finish the job in the continuation below. */

    const bool sans_shm_transport = transport_method_func(*(session_accept_result->m_target_session.client_app()));

    auto rcv = make_shared<Transport_hndl_receive>();
    rcv->m_chan = chan_for_rpc.async_io_obj();

    auto hndl_w_to_u_paf = newPromiseAndCrossThreadFulfiller<Error_code>();
    rcv->m_chan.hndl_rcv()->async_receive_native_handle
         ...
         (const Error_code& err_code, size_t)
    {
      ...
      (*hndl_w_to_u_fulfiller)->fulfill(Error_code{err_code});
    });

    m_kj_tasks.add
      (hndl_w_to_u_paf.promise
                      .then([this, enable_hndl_transport, sans_shm_transport,
                             rcv, // Keep the async-I/O channel and receive-target objects alive!
                             session_accept_result = std::move(session_accept_result), // Keep alive here too.
                             accept_result_fulfiller = std::move(accept_result_fulfiller)]
                              (const Error_code& err_code) mutable
    {
      // We are in thread U (the client's handle-bearing message arrived; or the channel got hosed).

      if (err_code || rcv->m_hndl.null())
      {
        FLOW_LOG_WARNING("rpc::Ctx_server [" << *this << "]: "
                         "In async-accept promise continuation: session-async-accept had succeeded; but receiving "
                         "the capnp-RPC transport handle over the client's first init-channel then failed "
                         "(code [" << err_code << "] [" << err_code.message() << "]; "
                         "null handle with no error would mean opposing peer violated protocol).  "
                         "Rejecting promise to inform our original caller.");
        accept_result_fulfiller->reject
          (Kj_exception{Kj_exception::Type::FAILED, __FILE__, __LINE__,
                        heapString(ostream_op_string("rpc::Ctx_server [", *this,
                                                     "]: session-async-accept had succeeded; but receiving the "
                                                       "capnp-RPC transport handle over the client's first "
                                                       "init-channel then failed (code [",
                                                     err_code, "] [",
                                                     err_code.message(),
                                                     "]; null handle with no error would mean "
                                                       "opposing peer violated protocol).").c_str())});
        return;
      }
